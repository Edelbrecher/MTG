...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    if ($_POST['action'] === 'create_deck') {
        ...
        // Clear commander for non-Commander formats
        if ($format !== 'Commander') {
            $commander = '';
        }
...
function generateEnhancedAIDeck($pdo, $deck_id, $strategy, $format, $quality, $user_id, $options = [], &$ai_log = []) {
    ...
    try {
        ...
        // Handle Commander selection for Commander format
        $commander_colors = [];
        if ($format === 'Commander') {
            // If no commander specified, try to pick one randomly from user's collection
            if (empty($commander)) {
                $ai_log[] = "🎲 Kein Commander gewählt - suche zufälligen Commander in Sammlung...";
                
                // Get all legendary creatures from user's collection
                $stmt = $pdo->prepare("
                    SELECT DISTINCT card_name, card_data 
                    FROM collections 
                    WHERE user_id = ? 
                    AND JSON_EXTRACT(card_data, '$.type_line') LIKE '%Legendary%' 
                    AND JSON_EXTRACT(card_data, '$.type_line') LIKE '%Creature%'
                    LIMIT 50
                ");
                $stmt->execute([$user_id]);
                $available_commanders = $stmt->fetchAll(PDO::FETCH_ASSOC);
                
                if (!empty($available_commanders)) {
                    $random_commander = $available_commanders[array_rand($available_commanders)];
                    $commander = $random_commander['card_name'];
                    $ai_log[] = "🎯 Zufälliger Commander ausgewählt: " . $commander;
                } else {
                    // Fallback: Create a basic commander deck with popular commanders
                    $fallback_commanders = [
                    ...
                    $commander = $fallback_commanders[array_rand($fallback_commanders)];
                    $ai_log[] = "⚠️ Keine legendären Kreaturen in Sammlung - verwende populären Commander: " . $commander;
                    $warnings[] = "Kein eigener Commander verfügbar - verwende " . $commander;
                }
            } else {
                $ai_log[] = "👑 Commander manuell gewählt: " . $commander;
            }
            ...
            if (!empty($commander_colors)) {
                ...
                // For fallback commanders, use a reasonable color identity
                if (strpos($commander, 'Atraxa') !== false) {
                    $commander_colors = ['W', 'U', 'B', 'G'];
                    $ai_log[] = "🎨 Color Identity (fallback): WUBG";
                } elseif (strpos($commander, 'Edgar') !== false) {
                    $commander_colors = ['W', 'B', 'R'];
                    $ai_log[] = "🎨 Color Identity (fallback): WBR";
                } else {
                    $commander_colors = ['W', 'U', 'B', 'R', 'G']; // Five-color fallback
                    $ai_log[] = "🎨 Color Identity (fallback): WUBRG";
                }
            }
        }
...
<body>
            ...
            <div class="card deck-builder-card">
                ...
                <form method="post" id="deckBuilderForm">
                    ...
                    <div id="commanderSection" style="display: none; margin-bottom: 1rem;">
                        ...
                        <small style="color: rgba(255,255,255,0.7); font-size: 0.8rem; line-height: 1.4;">
                            🎲 <strong>Tipp:</strong> Wenn kein Commander gewählt wird, wird automatisch ein zufälliger Commander aus Ihrer Sammlung ausgewählt
                        </small>
                    </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            // Ensure commander field is properly initialized
            const commanderSelect = document.getElementById('commanderSelect');
            const formatSelect = document.querySelector('select[name="format"]');
            
            // If no Commander format selected on load, disable commander field
            if (formatSelect.value !== 'Commander') {
        ...
        function handleFormatChange(format) {
            ...
            if (format === 'Commander') {
                ...
                commanderSelect.disabled = true;
                commanderSelect.value = ''; // Reset commander value for non-Commander formats
        ...
        document.getElementById('deckBuilderForm').addEventListener('submit', function(e) {
            ...
            // Commander-spezifische Validierung nur für Commander-Format
            if (format === 'Commander') {
                // Commander ist optional - kein Problem wenn leer
                if (deckSize !== 100) {
                    e.preventDefault();
                ...
                // Für andere Formate: Commander-Feld leeren und deaktivieren
                commanderSelect.value = '';
                commanderSelect.disabled = true;
            }
            
            // Format-spezifische Deck-Größen-Validierung
